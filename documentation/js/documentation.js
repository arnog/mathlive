...
export function generateMacrosDocumentation(macros) {
  macros = macros || debug.MACROS;
  let result = "<h3>Macros</h3><table style='table-layout:auto'>";
  result += '<tr><th>Example</th><th>Macro</th><th>Definition</th></tr>';

  Object.keys(macros).forEach((macro) => {
    let example = '\\' + macro;
    let macroWithParams = example;
    if (/#3/.test(macros[macro])) {
      macroWithParams += '{}{}{}';
      example += '{ab}{cd}{eg}';
    } else if (/#2/.test(macros[macro])) {
      macroWithParams += '{}{}';
      example += '{ab}{cd}';
    } else if (/#1/.test(macros[macro])) {
      macroWithParams += '{}';
      example += '{ab}';
    }

    result += '<tr>';
    result += '<td>$$' + example + '$$</td>';
    result += '<td><tt><strong>' + macroWithParams + '</strong></tt></td>';
    result += '<td><tt>' + macros[macro] + '</tt></td>';
    result += '</tr>';
  });
  result += '</table>';
  return result;
...
//     let result = '';
//     for (const color in Colors.NAMED_COLORS) {
//         if (Colors.NAMED_COLORS.hasOwnProperty(color)) {
//             result += '{\\color{' + color;
//             result += '}\\blacksquare}';
//         }
//     }
