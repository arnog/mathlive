...
import { KEYBOARDS, LAYERS, SHIFTED_KEYS } from './data';
import { VirtualKeyboard } from './virtual-keyboard';
import { MathfieldProxy } from './types';
import { hideVariantsPanel, makeVariants } from './variants';
...
export function makeKeyboardElement(keyboard: VirtualKeyboard): HTMLDivElement {
  ...
  let markup = SVG_ICONS;

  injectStylesheets();

  let keyboardIDs = keyboard.virtualKeyboards;
  ...
  const layers = {
    ...LAYERS,
    ...(keyboard.customVirtualKeyboards.layers ?? {}),
  };
  const keyboards = {
    ...KEYBOARDS,
    ...(keyboard.customVirtualKeyboards.keyboards ?? {}),
  };
  ...
  for (const keyboardName of keyboardList) {
    ...
    for (const layerName of keyboardLayers) {
      ...
      if (typeof layerMarkup === 'string')
        markup += expandLayerMarkup(keyboard, layerMarkup);
      markup += '</div>';
    }
  }
