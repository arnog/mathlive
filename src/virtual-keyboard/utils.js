...
exports.parentKeycap = exports.executeKeycapCommand = exports.normalizeKeycap = exports.KEYCAP_SHORTCUTS = exports.renderKeycap = exports.makeKeyboardElement = exports.releaseStylesheets = exports.makeSyntheticKeycaps = exports.makeEditToolbar = exports.normalizeLayout = void 0;
...
function jsonToCssProps(json) {
    ...
    return Object.entries(json)
        .map(function (_a) {
        var k = _a[0], v = _a[1];
        return "".concat(k, ":").concat(v, " !important");
    })
        .join(';');
}
function jsonToCss(json) {
    return Object.keys(json)
        .map(function (k) {
        return "".concat(k, " {").concat(jsonToCssProps(json[k]), "}");
    })
        .join('');
}
function latexToMarkup(latex) {
    ...
    var root = new atom_1.Atom({
        mode: 'math',
        type: 'root',
        body: (0, parser_1.parseLatex)(latex, {
            context: context,
            args: function (arg) {
                return arg === '@'
    ...
    var box = (0, box_1.coalesce)((0, inter_box_spacing_1.applyInterBoxSpacing)(new box_1.Box(root.render(context), { classes: 'ML__base' }), context));
    return (0, box_1.makeStruts)(box, { classes: 'ML__latex' }).toMarkup();
}
function normalizeLayer(layer) {
    var _a;
    if (Array.isArray(layer))
        return layer.map(function (x) { return normalizeLayer(x); }).flat();
    var result = typeof layer === 'string' ? { markup: layer } : layer;
    if ('rows' in result && Array.isArray(result.rows))
        result.rows = result.rows.map(function (row) { return row.map(function (x) { return normalizeKeycap(x); }); });
    (_a = result.id) !== null && _a !== void 0 ? _a : (result.id = 'ML__layer_' +
...
function alphabeticLayout() {
    ...
    var layoutName = keyboard.alphabeticLayout;
    if (layoutName === 'auto') {
        ...
        if (!layoutName || layoutName === 'auto') {
            layoutName =
                (_a = {
                    ...
                    sk: 'qwertz',
                    ch: 'qwertz'
                }[l10n_1.l10n.locale.slice(0, 2)]) !== null && _a !== void 0 ? _a : 'qwerty';
        }
    }
    ...
    var template = (_b = ALPHABETIC_TEMPLATE[layoutName]) !== null && _b !== void 0 ? _b : ALPHABETIC_TEMPLATE.qwerty;
    var rows = layoutName === 'azerty'
        ? [
            [
                { label: '1', variants: '1' },
    ...
    for (var _i = 0, template_1 = template; _i < template_1.length; _i++) {
        var templateRow = template_1[_i];
        var row = [];
        for (var _c = 0, templateRow_1 = templateRow; _c < templateRow_1.length; _c++) {
            var k = templateRow_1[_c];
            if (/[a-z]/.test(k)) {
                row.push({
                    label: k,
                    "class": 'hide-shift',
                    shift: {
                        label: k.toUpperCase(),
                        variants: (0, variants_1.hasVariants)(k.toUpperCase())
                            ? k.toUpperCase()
                            : undefined
                    },
                    variants: (0, variants_1.hasVariants)(k) ? k : undefined
                });
            }
            else if (k === '~') {
    ...
    return {
        ...
        layers: normalizeLayer({ rows: rows })
    };
}
function normalizeLayout(layout) {
    ...
    if (typeof layout === 'string') {
        console.assert(data_1.LAYOUTS[layout] !== undefined, "MathLive {{SDK_VERSION}}: unknown keyboard layout \"".concat(layout, "\""));
        return normalizeLayout(data_1.LAYOUTS[layout]);
    }
    var result;
    if ('rows' in layout && Array.isArray(layout.rows)) {
        console.assert(!('layers' in layout || 'markup' in layout), "MathLive {{SDK_VERSION}}: when providing a \"rows\" property, \"layers\" and \"markup\" are ignored");
        // Remove the `rows` key from `layout`
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        var rows = layout.rows, partialLayout = __rest(layout, ["rows"]);
        result = __assign(__assign({}, partialLayout), { layers: normalizeLayer({ rows: layout.rows }) });
    }
    else if ('markup' in layout && typeof layout.markup === 'string') {
        // Remove the `markup` key from `layout`
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        var markup = layout.markup, partialLayout = __rest(layout, ["markup"]);
        result = __assign(__assign({}, partialLayout), { layers: normalizeLayer(layout.markup) });
    }
    else {
        result = __assign({}, layout);
        if ('layers' in layout)
            result.layers = normalizeLayer(layout.layers);
        else {
            console.error("MathLive {{SDK_VERSION}}: provide either a \"rows\", \"markup\" or \"layers\" property");
        }
    }
    var hasShift = false;
    var hasEdit = false;
    for (var _i = 0, _a = result.layers; _i < _a.length; _i++) {
        var layer = _a[_i];
        if (layer.rows) {
            for (var _b = 0, _c = layer.rows.flat(); _b < _c.length; _b++) {
                var keycap = _c[_b];
                if (isShiftKey(keycap))
                    hasShift = true;
                var command = keycap.command;
...
function makeLayoutsToolbar(keyboard, index) {
    ...
    var markup = "<div class=\"left\">";
    if (keyboard.normalizedLayouts.length > 1) {
        for (var _i = 0, _c = keyboard.normalizedLayouts.entries(); _i < _c.length; _i++) {
            var _d = _c[_i], i = _d[0], l = _d[1];
            var layout = l;
            var classes = [i === index ? 'selected' : 'layer-switch'];
            if (layout.tooltip)
                classes.push('MLK__tooltip');
            if (layout.labelClass)
                classes.push.apply(classes, layout.labelClass.split(' '));
            markup += "<div class=\"".concat(classes.join(' '), "\"");
            if (layout.tooltip) {
                markup +=
                    " data-tooltip='" + ((_a = (0, l10n_1.localize)(layout.tooltip)) !== null && _a !== void 0 ? _a : layout.tooltip) + "' ";
            }
            if (i !== index)
                markup += "data-layer=\"".concat(layout.layers[0].id, "\"");
            markup += ">".concat((_b = layout.label) !== null && _b !== void 0 ? _b : 'untitled', "</div>");
        }
    }
    markup += '</div>';
    return markup;
...
function makeEditToolbar(options, mathfield) {
    var result = '';
    var toolbarOptions = options.editToolbar;
    ...
    if (mathfield.selectionIsCollapsed)
        ...
        availableActions.push('cutToClipboard', 'copyToClipboard', 'pasteFromClipboard');
    }
    var actionsMarkup = {
        undo: "<div class='action ".concat(mathfield.canUndo === false ? 'disabled' : '', "'\n          data-command='\"undo\"'\n          data-tooltip='").concat((0, l10n_1.localize)('tooltip.undo'), "'>\n          <svg><use xlink:href='#svg-undo' /></svg>\n      </div>"),
        redo: "<div class='action ".concat(mathfield.canRedo === false ? 'disabled' : '', "'\n          data-command='\"redo\"'\n          data-tooltip='").concat((0, l10n_1.localize)('tooltip.redo'), "'>\n          <svg><use xlink:href='#svg-redo' /></svg>\n      </div>"),
        cutToClipboard: "\n        <div class='action'\n            data-command='\"cutToClipboard\"'\n            data-tooltip='".concat((0, l10n_1.localize)('tooltip.cut to clipboard'), "'>\n            <svg><use xlink:href='#svg-cut' /></svg>\n        </div>\n    "),
        copyToClipboard: "\n        <div class='action'\n            data-command='\"copyToClipboard\"'\n            data-tooltip='".concat((0, l10n_1.localize)('tooltip.copy to clipboard'), "'>\n            <svg><use xlink:href='#svg-copy' /></svg>\n        </div>\n    "),
        pasteFromClipboard: "\n        <div class='action'\n            data-command='\"pasteFromClipboard\"'\n            data-tooltip='".concat((0, l10n_1.localize)('tooltip.paste from clipboard'), "'>\n            <svg><use xlink:href='#svg-paste' /></svg>\n        </div>\n    ")
    };
    // The right hand side of the toolbar, with the copy/undo/redo commands
    result += availableActions.map(function (action) { return actionsMarkup[action]; }).join('');
    return result;
}
exports.makeEditToolbar = makeEditToolbar;
...
exports.makeSyntheticKeycaps = makeSyntheticKeycaps;
function makeSyntheticKeycap(element) {
    ...
    if (!element.innerHTML) {
        var _a = renderKeycap(keycap), markup = _a[0], _ = _a[1];
        element.innerHTML = globalThis.MathfieldElement.createHTML(markup);
    }
}
function injectStylesheets() {
    (0, stylesheet_1.injectStylesheet)('virtual-keyboard');
...
function releaseStylesheets() {
    ...
    (0, stylesheet_1.releaseStylesheet)('virtual-keyboard');
}
exports.releaseStylesheets = releaseStylesheets;
...
function makeKeyboardElement(keyboard) {
    ...
    plate.innerHTML = globalThis.MathfieldElement.createHTML(SVG_ICONS +
        keyboard.normalizedLayouts
            .map(function (x, i) { return makeLayout(keyboard, x, i); })
            .join(''));
    ...
    var toolbars = result.querySelectorAll('.ML__edit-toolbar');
    if (toolbars) {
        for (var _i = 0, toolbars_1 = toolbars; _i < toolbars_1.length; _i++) {
            var toolbar_1 = toolbars_1[_i];
            toolbar_1.addEventListener('click', function (ev) {
                var _a, _b;
                var target = ev.target;
                var command = '';
                while (target && !command) {
                    command = (_a = target === null || target === void 0 ? void 0 : target.getAttribute('data-command')) !== null && _a !== void 0 ? _a : '';
                    target = (_b = target === null || target === void 0 ? void 0 : target.parentElement) !== null && _b !== void 0 ? _b : null;
                }
                if (command)
    ...
    makeSyntheticKeycaps(result.querySelectorAll('.MLK__keycap, .action, .fnbutton, .bigfnbutton'));
    var layerElements = result.querySelectorAll('.MLK__layer');
    ...
    for (var _a = 0, layerElements_1 = layerElements; _a < layerElements_1.length; _a++) {
        var x = layerElements_1[_a];
        x.addEventListener('pointerdown', function (evt) { return evt.preventDefault(); });
    }
    // Restore the last active keyboards, or pick the first one
    keyboard.currentLayer = keyboard.latentLayer;
    return result;
}
exports.makeKeyboardElement = makeKeyboardElement;
function makeLayout(keyboard, layout, index) {
    var markup = [];
    if (!('layers' in layout))
        return '';
    for (var _i = 0, _a = layout.layers; _i < _a.length; _i++) {
        var layer = _a[_i];
        markup.push("<div tabindex=\"-1\" class=\"MLK__layer\" id=\"".concat(layer.id, "\">"));
...
function makeLayer(keyboard, layer) {
    ...
    if (typeof layer.style === 'string')
        layerMarkup += "<style>".concat(layer.style, "</style>");
    else if (typeof layer.style === 'object')
        layerMarkup += "<style>".concat(jsonToCss(layer.style), "</style>");
    if (layer.backdrop)
        layerMarkup += "<div class='".concat(layer.backdrop, "'>");
    if (layer.container)
        layerMarkup += "<div class='".concat(layer.container, "'>");
    if (layer.rows) {
        layerMarkup += "<div class=MLK__rows>";
        for (var _i = 0, _a = layer.rows; _i < _a.length; _i++) {
            var row = _a[_i];
            layerMarkup += "<div dir=\"ltr\" class=MLK__row>";
            for (var _b = 0, row_1 = row; _b < row_1.length; _b++) {
                var keycap = row_1[_b];
                if (keycap) {
                    var keycapId = keyboard.registerKeycap(keycap);
                    var _c = renderKeycap(keycap), markup = _c[0], cls = _c[1];
                    if (/(^|\s)separator/.test(cls))
                        layerMarkup += "<div class=\"".concat(cls, "\"");
                    else
                        layerMarkup += "<div tabindex=\"-1\" id=\"".concat(keycapId, "\" class=\"").concat(cls, "\"");
                    if (keycap.tooltip)
                        layerMarkup += " data-tooltip=\"".concat(keycap.tooltip, "\"");
                    layerMarkup += ">".concat(markup, "</div>");
                }
            }
            layerMarkup += "</div>";
        }
...
function renderKeycap(keycap, options) {
    var _a, _b, _c, _d, _e, _f, _g;
    if (options === void 0) { options = { shifted: false }; }
    var markup = '';
    var cls = (_a = keycap["class"]) !== null && _a !== void 0 ? _a : '';
    ...
    if (options.shifted && 'shift' in keycap) {
        ...
        if (typeof keycap.shift === 'string')
            markup = latexToMarkup(keycap.shift);
        else if (typeof keycap.shift === 'object') {
            markup = keycap.shift.label
                ? keycap.shift.label
                : // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
                    (_b = (latexToMarkup(keycap.shift.latex || keycap.shift.insert || '') ||
                        keycap.shift.key)) !== null && _b !== void 0 ? _b : '';
        }
        if (typeof keycap.shift === 'object')
            cls = (_d = (_c = keycap.shift["class"]) !== null && _c !== void 0 ? _c : keycap["class"]) !== null && _d !== void 0 ? _d : '';
    }
    else {
        //
        ...
        markup = keycap.label
            ? keycap.label
            : // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
                (_e = (latexToMarkup(keycap.latex || keycap.insert || '') || keycap.key)) !== null && _e !== void 0 ? _e : '';
        if (keycap.shift) {
            // There is a shift version of this keycap, render a small label
            var shiftLabel = void 0;
            if (typeof keycap.shift === 'string')
                ...
                shiftLabel =
                    // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
                    (_f = (latexToMarkup(keycap.shift.latex || keycap.shift.insert || '') ||
                        keycap.shift.key)) !== null && _f !== void 0 ? _f : '';
            }
            markup += "<span class=\"MLK__shift\">".concat(shiftLabel, "</span>");
        }
        if (keycap.aside)
            markup += "<aside>".concat(keycap.aside, "</aside>");
    }
    ...
    if (!/\bw[0-9]+\b/.test(cls) && keycap.width) {
        cls +=
            (_g = { 0: ' w0', 0.5: ' w5', 1.5: ' w15', 2.0: ' w20', 5.0: ' w50' }[keycap.width]) !== null && _g !== void 0 ? _g : '';
    }
    return [markup, cls || 'MLK__keycap'];
}
exports.renderKeycap = renderKeycap;
exports.KEYCAP_SHORTCUTS = {
    '[left]': {
...
function normalizeKeycap(keycap) {
    ...
    if (typeof keycap === 'string') {
        if (keycap === '[.]' &&
            ...
            keycap = '[,]';
        if (!exports.KEYCAP_SHORTCUTS[keycap])
            return { latex: keycap };
        keycap = { label: keycap };
    }
    var shortcut = undefined;
    if ('label' in keycap && keycap.label && exports.KEYCAP_SHORTCUTS[keycap.label]) {
        shortcut = __assign(__assign(__assign({}, exports.KEYCAP_SHORTCUTS[keycap.label]), keycap), { label: exports.KEYCAP_SHORTCUTS[keycap.label].label });
    }
    if ('key' in keycap && keycap.key && exports.KEYCAP_SHORTCUTS[keycap.key]) {
        shortcut = __assign(__assign(__assign({}, exports.KEYCAP_SHORTCUTS[keycap.key]), keycap), { key: exports.KEYCAP_SHORTCUTS[keycap.key].key });
    }
    if (!shortcut)
        return keycap;
    if (shortcut.command === 'insertDecimalSeparator')
        shortcut.label = (_a = globalThis.MathfieldElement.decimalSeparator) !== null && _a !== void 0 ? _a : '.';
    if (shortcut.tooltip === undefined ||
        shortcut.tooltip === null ||
        shortcut.tooltip === false) {
    ...
    if (shortcut.aside === undefined ||
        shortcut.aside === null ||
        shortcut.aside === false)
        delete shortcut.aside;
    if (shortcut.variants === undefined ||
        shortcut.variants === null ||
        shortcut.variants === false)
...
exports.normalizeKeycap = normalizeKeycap;
...
function handlePointerDown(ev) {
    ...
    //
    var layerButton = ev.target;
    while (layerButton && !layerButton.getAttribute('data-layer'))
        layerButton = layerButton.parentElement;
    if (layerButton) {
        keyboard.currentLayer = (_a = layerButton.getAttribute('data-layer')) !== null && _a !== void 0 ? _a : '';
        ev.preventDefault();
        return;
    }
    //
    ...
    //
    var target = parentKeycap(ev.target);
    if (!(target === null || target === void 0 ? void 0 : target.id))
        return;
    var keycap = keyboard.getKeycap(target.id);
    if (!keycap)
        return;
    ...
    target.addEventListener('pointerenter', handleVirtualKeyboardEvent(controller), { capture: true, signal: signal });
    target.addEventListener('pointerleave', handleVirtualKeyboardEvent(controller), { capture: true, signal: signal });
    target.addEventListener('pointercancel', handleVirtualKeyboardEvent(controller), { signal: signal });
    ...
    if (keycap.variants) {
        ...
        pressAndHoldTimer = setTimeout(function () {
            if (target.classList.contains('is-pressed')) {
                ...
                (0, variants_1.showVariantsPanel)(target, function () {
                    controller.abort();
...
function executeKeycapCommand(keycap) {
    ...
    (_a = virtual_keyboard_1.VirtualKeyboard.singleton) === null || _a === void 0 ? void 0 : _a.executeCommand(command);
}
exports.executeKeycapCommand = executeKeycapCommand;
function isKeycapElement(el) {
    if (el.nodeType !== 1)
        return false;
    var classes = el.classList;
    return (classes.contains('MLK__keycap') ||
...
function parentKeycap(el) {
    ...
    return (_a = node) !== null && _a !== void 0 ? _a : undefined;
}
exports.parentKeycap = parentKeycap;
