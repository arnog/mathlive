...
exports.smartMode = exports.removeIsolatedSpace = void 0;
...
function convertLastAtomsToMath(model, count, until) {
    ...
    while (!done) {
        var atom = model.at(i);
            ...
            count === 0 ||
                ...
                (typeof until === 'function' && !until(atom));
        if (!done) {
            data.push(atom_class_1.Atom.serialize([atom], { defaultMode: 'math' }));
            atom.mode = 'math';
        }
        i -= 1;
        count -= 1;
    ...
    model.contentDidChange({ data: (0, tokenizer_1.joinLatex)(data), inputType: 'insertText' });
}
/**
...
function removeIsolatedSpace(model) {
    ...
    if (model.at(i).mode === 'text' &&
        ...
        var save = model.silenceNotifications;
...
exports.removeIsolatedSpace = removeIsolatedSpace;
/**
...
function smartMode(mathfield, keystroke, evt) {
    ...
    var context = getTextBeforePosition(model) + c;
    if (mathfield.model.mode === 'text') {
        ...
        if (lFence &&
            parent_1 instanceof leftright_1.LeftRightAtom &&
            parent_1.leftDelim === lFence)
            return true;
        ...
        if (keystroke === '[Space]') {
            convertLastAtomsToText(model, undefined, function (a) {
                return /[a-z][:,;.]$/.test(a.value);
            });
            return true;
        }
        if (/[a-zA-Z]{3,}$/.test(context) && !/(dxd|abc|xyz|uvw)$/.test(context)) {
            ...
            convertLastAtomsToText(model, undefined, function (a) { return /[a-zA-Z]/.test(a.value); });
            return true;
        }
        ...
        if (/(\u0393|\u0394|\u0398|\u039B|\u039E|\u03A0|\u03A3|\u03A5|\u03A6|\u03A8|\u03A9|[\u03B1-\u03C9]|\u03D1|\u03D5|\u03D6|\u03F1|\u03F5){3,}$/u.test(context) &&
            ...
            // Convert them to text.
            convertLastAtomsToText(model, undefined, function (a) {
                return /(:|,|;|.|\u0393|\u0394|\u0398|\u039B|\u039E|\u03A0|\u03A3|\u03A5|\u03A6|\u03A8|\u03A9|[\u03B1-\u03C9]|\u03D1|\u03D5|\u03D6|\u03F1|\u03F5)/u.test(a.value);
            });
            return true;
        }
        if (c === '?') {
