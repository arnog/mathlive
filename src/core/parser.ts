...
export class Parser {
  ...
  scanDelim(): string | null {
    ...
    if (info.type === 'mopen' || info.type === 'mclose') return delim;
  ...
  parseCommand(command: string): Atom[] | null {
    if (command === '\\placeholder') {
      const placeholder = new PlaceholderAtom(this.context, {
        mode: this.parseMode,
  ...
  scanMacro(macro: string): Atom | null {
    ...
    return new MacroAtom(macro, this.context, {
      ...
      body: parseLatex(def, this.context, {
        parseMode: this.parseMode,
  ...
  parseToken(): boolean {
    ...
    // for example
    if (isArray<Atom>(result)) this.mathlist.push(...result);
