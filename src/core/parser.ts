...
export class Parser {
  ...
  tabularMode = false; // For arrays, matrix, etc. different parsing rules apply.
  ...
  mathstyle: MathstyleName = 'displaystyle';
  registers: string[]; // A list of the valid register names
  ...
  parseLimits(): boolean {
    ...
    const opAtom =
      this.atoms.length > 0 ? this.atoms[this.atoms.length - 1] : null;
...
export function parseLatex(
  ...
  parser.parseMode = options?.parseMode ?? 'math';
  if (options?.smartFence ?? false) parser.smartFence = true;
