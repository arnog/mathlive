...
exports.codePointToLatex = exports.unicodeToMathVariant = exports.mathVariantToUnicode = exports.UNICODE_TO_LATEX = void 0;
/**
 * Map some unicode characters to their LaTeX equivalent
 */
// prettier-ignore
exports.UNICODE_TO_LATEX = {
    0x003c: '\\lt',
    0x003e: '\\gt',
    0x006f: 'o',
    0x0026: '\\&',
...
function mathVariantToUnicode(char, variant, style) {
    ...
    if (codepoint === undefined)
        return char;
    for (var _i = 0, MATH_UNICODE_BLOCKS_1 = MATH_UNICODE_BLOCKS; _i < MATH_UNICODE_BLOCKS_1.length; _i++) {
        var block = MATH_UNICODE_BLOCKS_1[_i];
        if (!variant || block.variant === variant) {
            if (!style || block.style === style) {
                if (codepoint >= block.offset && codepoint < block.offset + block.len) {
                    var result = block.start + codepoint - block.offset;
...
exports.mathVariantToUnicode = mathVariantToUnicode;
function unicodeToMathVariant(codepoint) {
    ...
    for (var _i = 0, MATH_UNICODE_BLOCKS_2 = MATH_UNICODE_BLOCKS; _i < MATH_UNICODE_BLOCKS_2.length; _i++) {
        var block = MATH_UNICODE_BLOCKS_2[_i];
...
exports.unicodeToMathVariant = unicodeToMathVariant;
/**
...
function codePointToLatex(c) {
    ...
    if (c.length > 1)
        return undefined;
    var codepoint = (_a = c.codePointAt(0)) !== null && _a !== void 0 ? _a : 0;
    var latex = exports.UNICODE_TO_LATEX[codepoint];
    if (latex)
        return latex;
    var _b = unicodeToMathVariant(codepoint), char = _b.char, variant = _b.variant, style = _b.style;
    if (!variant && !style)
        return undefined;
    latex = char;
    switch (variant) {
        case 'double-struck':
            latex = "\\mathbb{".concat(latex, "}");
            break;
        case 'fraktur':
            latex = "\\mathfrak{".concat(latex, "}");
            break;
        case 'script':
            latex = "\\mathscr{".concat(latex, "}");
            break;
        case 'sans-serif':
            latex = "\\mathsf{".concat(latex, "}");
            break;
        case 'monospace':
            latex = "\\mathtt{".concat(latex, "}");
            break;
        case 'calligraphic':
            latex = "\\mathcal{".concat(latex, "}");
            break;
    }
    switch (style) {
        case 'bold':
            latex = "\\mathbf{".concat(latex, "}");
            break;
        case 'italic':
            latex = "\\mathit{".concat(latex, "}");
            break;
        case 'bolditalic':
            latex = "\\mathbfit{".concat(latex, "}");
            break;
    }
    return latex;
}
exports.codePointToLatex = codePointToLatex;
