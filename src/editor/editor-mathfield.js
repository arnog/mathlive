 ...
 * {Object} config
 * 
 * {*} config.substituteTextArea - A function that returns a focusable element
 * that can be used to capture text input.
 * 
 * {mathfieldCallback} config.onFocus - Invoked when the mathfield has been focused
 * 
 * {mathfieldCallback} config.onBlur - Invoked when the mathfield has been blurred
 * 
 * {boolean} config.overrideDefaultInlineShorctus - If true, the default 
 * inline shortcuts (e.g. 'p' + 'i' = 'π') are ignored. Default false.
 * 
 * {Object} config.inlineShortcuts - A map of shortcuts -> replacement value.
 * For example { 'pi': '\\pi'}. If overrideDefaultInlineShorctus is false, 
 * these shortcuts are applied after any default ones, and can therefore replace
 * them.
 * 
 * {mathfieldWithDirectionCallback} config.onMoveOutOf - A handler called when 
 ...
 * 
 * {mathfieldWithDirectionCallback} config.onTabOutOf - A handler called when 
 ...
 * 
 * {mathfieldWithDirectionCallback} config.onDeleteOutOf - A handler called when 
 * deleting an item would cause the insertion point to leave the mathfield.
 * By default, nothing happens. @todo
 * 
 * {mathfieldWithDirectionCallback} config.onSelectOutOf - A handler called when 
 * the selection is extended so that it would cause the insertion point to 
 * leave the mathfield.
 * By default, nothing happens. @todo
 * 
 * {mathfieldCallback} config.onUpOutOf - A handler called when 
 * the up arrow key is pressed with no element to navigate to.
 * By default, nothing happens. @todo
 * 
 * {mathfieldCallback} config.onDownOutOf - A handler called when 
 * the up down key is pressed with no element to navigate to.
 * By default, nothing happens. @todo
 * 
 * {mathfieldCallback} config.onEnter - A handler called when 
 * the enter/return key is pressed and it is not otherwise handled. @todo
 * 
 * {mathfieldCallback} config.onContentWillChange - A handler called when 
 * just before the content is about to be changed. @todo
 * 
 * {mathfieldCallback} config.onContentDidChange - A handler called when 
 * just after the content has been changed.@todo
 * 
 * {mathfieldCallback} config.onSelectionWillChange - A handler called when 
